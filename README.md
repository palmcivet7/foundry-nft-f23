# Foundry NFTs

This project was made following along with the Cyfrin Foundry course and covers the basics of NFTs.

Type the following command in the directory of an svg file:

```
base64 -i example.svg
```

to get a string representing the svg file.
Then paste the string after

```
data:image/svg+xml;base64,
```

and paste into a browser to see the file.

Example:

```
data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI1MDAiIGhlaWdodD0iNTAwIj4KPHRleHQgeD0iMCIgeT0iMTUiIGZpbGw9IndoaXRlIj5IZWxsbywgeW91IGRlY29kZWQgdGhpcyBzdmcgdGV4dC48L3RleHQ+Cjwvc3ZnPg==
```

happySvg:

```
data:image/svg+xml;base64,
```

sadSvg:

```
data:image/svg+xml;base64,
```
